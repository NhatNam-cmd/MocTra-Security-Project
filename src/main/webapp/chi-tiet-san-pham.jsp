...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const cartForm = document.querySelector('form[action="gio-hang"]');
        if (cartForm) {
            cartForm.addEventListener('submit', function(e) {
                e.preventDefault();

                const formData = new URLSearchParams(new FormData(cartForm));
    ...
    function showToast(message, isError = false) {
        ...
        var text = x.querySelector("span");

        text.innerText = message;

        if (isError) {
            x.style.borderLeft = "5px solid #f44336";
            icon.className = "fa-solid fa-circle-exclamation";
            icon.style.color = "#f44336";
        } else {
            x.style.borderLeft = "5px solid #4CAF50";
