...
public class PromotionDAO {
    ...
    public List<Integer> findConflictingProductIds(List<Integer> productIds) {
        List<Integer> conflictIds = new ArrayList<>();
        if (productIds == null || productIds.isEmpty()) return conflictIds;


        StringBuilder sqlBuilder = new StringBuilder();
        sqlBuilder.append("SELECT DISTINCT pi.product_id FROM promotion_items pi ");
        sqlBuilder.append("JOIN promotions p ON pi.promotion_id = p.id ");
        sqlBuilder.append("WHERE p.active = 1 ");
        sqlBuilder.append("AND p.start_date <= NOW() AND p.end_date >= NOW() ");
        sqlBuilder.append("AND pi.product_id IN (");
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                conflictIds.add(rs.getInt("product_id"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
